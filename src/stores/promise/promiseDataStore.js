  ...
  devtools(
    immer((set) => ({
      ...
      actions: {
        ...
        hasDataUntil: (step) => {
          const state = promiseDataStore.getState();
          const InfoState = !!state.name && !!state.description;
          switch (step) {
            case 'date':
              return InfoState;
            case 'location':
              return InfoState && !!state.date;
            case 'schedule':
              return InfoState && !!state.date && !!state.nearestSubwayStation;
            default:
              return false;
          }
        },
        hasNearestSubwayStationData: () => !!promiseDataStore.getState().nearestSubwayStation,
